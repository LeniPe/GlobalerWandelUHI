...
### Skript zur Berechnung der UHI in Marburg für das Jahr 2018
...
lstdir <- "G:/Studium_Geographie/MSc_Physische_Geographie/Kurse/Globaler Wandel/Stadtklima in Marburg/Lokaler_Teil/LST_Daten/2018"
...
outdir <- 'G:/Studium_Geographie/MSc_Physische_Geographie/Kurse/Globaler Wandel/Stadtklima in Marburg/Lokaler_Teil/UHI_Marburg_Ergebnisse/2018/'

LST_Marburg <-raster(paste0(lstdir,"/LST_2018_mean_AOI.tif"))
LST_Marburg
plot(LST_Marburg)
...
LUC_Marburg <- raster(paste0(clcdir,"/CLC_2018_reclass.tif"))
LUC_Marburg
plot(LUC_Marburg)
...
plot(Marburg_shp, add = T)
outfileBuffer <- 'Marburg_2018_buffer.shp'
...
outfileBufferNew <- 'Marburg_2018_buffer_area_without_Marburg.shp'
...
LST_City_LUC4 <- which(LST_LUC_Marburg_Daten$CLC_2018_reclass==1)
LST_City_LUC4


LSTMean_City_LUC1_2 <- mean(LST_LUC_Marburg_Daten$LST_2018_mean_AOI[LST_City_LUC4])
LSTMean_City_LUC1_2

### die gemittelte Temperatur aller Pixel mit babauter Fläche beträgt 302.3709 Kelvin
...
LST_Rural_LUC2to4 <- which(LST_LUC_Data_Rural$CLC_2018_reclass > 1)
LST_Rural_LUC2to4


LSTMean_Rural_LUC2to4 <- mean(LST_LUC_Data_Rural$LST_2018_mean_AOI[LST_Rural_LUC2to4], na.rm = TRUE)
LSTMean_Rural_LUC2to4

### die gemittelte Temperatur aller Pixel ohne babauter Fläche beträgt 301.6836 Kelvin

UHIE <- LSTMean_City_LUC1_2 - LSTMean_Rural_LUC2to4
UHIE

### Der UHI-Effekt für Marburg im Sommer 2018 beträgt 0.6872909 Kelvin
...
outfile_CityLST <- 'LST_Marburg_2018_Stadtgebiet.tif'
...
outfile_CityLUC <- 'LUC_Marburg_2018_Stadtgebiet.tif'
...
outfile_City_UHIE <- 'UHIE_Marburg_2018_LUC1_Sommer.tif'
...
outfile_City_all_UHIE <- 'UHIE_Marburg_2018_LUC_all_Sommer.tif'
